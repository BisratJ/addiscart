...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (redirectPath) {
        ...
        router.push('/stores'); // Default redirect to stores page after login
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 via-green-50/30 to-gray-50 py-8 sm:py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-md w-full relative z-10">
        ...
        <div className="text-center mb-6 sm:mb-8">
          <Link 
            href="/" 
            className="inline-flex items-center gap-2 mb-3 sm:mb-4 group/logo hover:opacity-80 transition-opacity active:scale-95 touch-manipulation"
          ...
          >
            <div className="relative">
              ...
              <div className="relative w-9 h-9 sm:w-10 sm:h-10 bg-gradient-to-br from-green-500 via-emerald-500 to-lime-400 rounded-lg flex items-center justify-center shadow-lg">
                <ShoppingCart className="w-5 h-5 sm:w-6 sm:h-6 text-white" strokeWidth={2.5} />
              </div>
            </div>
            <span className="font-bold text-xl sm:text-2xl bg-gradient-to-r from-green-600 via-emerald-600 to-lime-600 bg-clip-text text-transparent">Addiscart</span>
          </Link>
          <p className="text-[10px] sm:text-xs text-gray-500 mb-4 sm:mb-6">← Click logo to return home</p>
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">Welcome back</h2>
          <p className="text-sm sm:text-base text-gray-600">Sign in to continue your shopping</p>
        </div>
        
        {/* Login Card */}
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-2xl border border-gray-200/50 p-6 sm:p-8">
          {error && (
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-2 sm:gap-3 px-4 py-2.5 sm:py-3 border-2 border-gray-200 rounded-xl hover:border-gray-300 hover:bg-gray-50 transition-all duration-200 mb-4 sm:mb-6 group disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 touch-manipulation"
          >
            {isLoading ? (
              <>
